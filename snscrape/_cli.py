...
def _dump_locals_on_exception():
    try:
        ...
        if len(trace) >= 2:
            ...
            logger.fatal(
                f"Dumped stack and locals to {name}",
                extra={"_snscrapeSuppressDumpLocals": True},
            )
        raise
...
class CitationAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, optionString):
        try:
            ...
            print(
                "Error: could not find snscrape installation. --citation does not work without the package being installed.",
                file=sys.stderr,
            )
            parser.exit(1)
        ...
        if ".dev" in m["version"]:
            print()
            print(
                "WARNING! You are running a development version. The date range may be incorrect. Please adjust the upper end of the range to the year of the commit."
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"snscrape {snscrape.version.__version__}",
    )
    parser.add_argument(
        "--citation",
        action=CitationAction,
        nargs=0,
        help="Display recommended citation information and exit",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    group.add_argument(
        "--jsonl", dest="jsonl", action="store_true", default=False, help="Output JSONL"
    )
    parser.add_argument(
        "--with-entity",
        dest="withEntity",
        action="store_true",
        default=False,
        help="Include the entity (e.g. user, channel) as the first output item",
    )
    parser.add_argument(
        "--since",
    ...
    for scraper, cls in sorted(scrapers.items()):
        subparser = subparsers.add_parser(
            cls.name, help="", formatter_class=argparse.ArgumentDefaultsHelpFormatter
        )
        ...
        try:
            ...
            for i, item in enumerate(scraper.get_items(), start=1):
                ...
                if args.maxResults and i >= args.maxResults:
                    ...
                    if args.progress:
                        print(
                            f"Stopped scraping after {i} results due to --max-results",
                            file=sys.stderr,
                        )
                    break
            else:
