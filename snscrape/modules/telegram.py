...
class TelegramPost(snscrape.base.Item):
    ...
    outlinksss = snscrape.base._DeprecatedProperty(
        "outlinksss", lambda self: " ".join(self.outlinks), "outlinks"
    )
...
class Channel(snscrape.base.Item):
    ...
    videosGranularity = snscrape.base._DeprecatedProperty(
        "videosGranularity", lambda self: self.videos.granularity, "videos.granularity"
    )
    linksGranularity = snscrape.base._DeprecatedProperty(
        "linksGranularity", lambda self: self.links.granularity, "links.granularity"
    )
    filesGranularity = snscrape.base._DeprecatedProperty(
        "filesGranularity", lambda self: self.files.granularity, "files.granularity"
    )
...
class TelegramChannelScraper(snscrape.base.Scraper):
    ...
    def _soup_to_items(self, soup, pageUrl, onlyUsername=False):
        ...
        for post in reversed(posts):
            if onlyUsername:
                ...
                return
            dateDiv = post.find("div", class_="tgme_widget_message_footer").find(
                "a", class_="tgme_widget_message_date"
            )
            rawUrl = dateDiv["href"]
            if (
                not rawUrl.startswith("https://t.me/")
                or sum(x == "/" for x in rawUrl) != 4
                or rawUrl.rsplit("/", 1)[1].strip("0123456789") != ""
            ):
                _logger.warning(f"Possibly incorrect URL: {rawUrl!r}")
            url = rawUrl.replace("//t.me/", "//t.me/s/")
            date = datetime.datetime.strptime(
                dateDiv.find("time", datetime=True)["datetime"]
                .replace("-", "", 2)
                .replace(":", ""),
                "%Y%m%dT%H%M%S%z",
            )
    ...
    def _get_entity(self):
        ...
        try:
            ...
            kwargs["username"] = channelInfoDiv.find(
                "div", class_="tgme_channel_info_header_username"
            ).text[
                1:
            ]  # Remove @
        ...
        def parse_num(s):
            s = s.replace(" ", "")
            if s.endswith("M"):
                return int(float(s[:-1]) * 1e6), 10 ** (
                    6 if "." not in s else 6 - len(s[:-1].split(".")[1])
                )
            elif s.endswith("K"):
                return int(float(s[:-1]) * 1000), 10 ** (
                    3 if "." not in s else 3 - len(s[:-1].split(".")[1])
                )
            else:
                return int(s), 1
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            "channel",
            type=snscrape.base.nonempty_string("channel"),
            help="A channel name",
        )
