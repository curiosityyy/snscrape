...
class User(snscrape.base.Item):
    ...
    followersGranularity = snscrape.base._DeprecatedProperty(
        "followersGranularity",
        lambda self: self.followers.granularity,
        "followers.granularity",
    )
    postsGranularity = snscrape.base._DeprecatedProperty(
        "postsGranularity", lambda self: self.posts.granularity, "posts.granularity"
    )
    photosGranularity = snscrape.base._DeprecatedProperty(
        "photosGranularity", lambda self: self.photos.granularity, "photos.granularity"
    )
    tagsGranularity = snscrape.base._DeprecatedProperty(
        "tagsGranularity", lambda self: self.tags.granularity, "tags.granularity"
    )
    followingGranularity = snscrape.base._DeprecatedProperty(
        "followingGranularity",
        lambda self: self.following.granularity,
...
class VKontakteUserScraper(snscrape.base.Scraper):
    ...
    def _date_span_to_date(self, dateSpan):
        ...
        if match := re.match(
            r"^(?P<day>\d+)\s+(?P<month>" + "|".join(_months) + r")\s+(?P<year>\d{4})$",
            dateSpan.text,
        ):
            # Date only
            return datetime.date(
                int(match.group("year")),
                _months.index(match.group("month")) + 1,
                int(match.group("day")),
            )
        if dateSpan.text not in (
            "video",
            "photo",
        ):  # Silently ignore video and photo reposts which have no original date attached
    ...
    def _post_div_to_item(self, post, isCopy=False):
        ...
        assert (
            (
                url.startswith("https://vk.com/wall")
                or (
                    isCopy
                    and (
                        url.startswith("https://vk.com/video")
                        or url.startswith("https://vk.com/photo")
                    )
                )
            )
            and "_" in url
        ...
        if (
            ...
            and (mediaLinkA := mediaLinkDiv.find("a", class_="media_link__title"))
            and (href := self._away_a_to_url(mediaLinkA))
            and href not in outlinks
        ):
        ...
        if (
            ...
            and "class" in thumbsDiv.parent.attrs
            and "copy_quote" in thumbsDiv.parent.attrs["class"]
        ):  # Skip post quotes
            photos = []
            for a in thumbsDiv.find_all("a", class_="page_post_thumb_wrap"):
                ...
                # From here on: photo
                if (
                    "onclick" not in a.attrs
                    or not a["onclick"].startswith("return showPhoto('")
                    or '{"temp":' not in a["onclick"]
                    or not a["onclick"].endswith("}, event)")
                ):
                    _logger.warning(
                ...
                for x in singleLetterKeys:
                    ...
                    if not photoObj["temp"][x_][0].startswith("https://"):
                        photoObj["temp"][x_][
                            0
                        ] = f'{photoObj["temp"]["base"]}{photoObj["temp"][x_][0]}'
                if (
                    any(
                    ...
                    _logger.warning(
                        f"Photo thumb wrap on {url} has unexpected data structure, skipping"
                    )
                    continue
                photoVariants = []
                for x in singleLetterKeys:
                    x_ = f"{x}_"
                    photoVariants.append(
                        PhotoVariant(
                            url=f'{photoObj["temp"][x_][0]}.jpg'
                            if ".jpg" not in photoObj["temp"][x_][0]
                            else photoObj["temp"][x_][0],
                            width=photoObj["temp"][x_][1],
                            height=photoObj["temp"][x_][2],
                        )
                    )
                photoUrl = (
                    f'https://vk.com{a["href"]}'
                    if "href" in a.attrs
                    and a["href"].startswith("/photo")
                    and a["href"][6:].strip("0123456789-_") == ""
                    else None
                )
                photos.append(Photo(variants=photoVariants, url=photoUrl))
        quotedPost = (
            self._post_div_to_item(quoteDiv, isCopy=True)
            if (quoteDiv := post.find("div", class_="copy_quote"))
            else None
        )
    ...
    def _initial_page(self):
        if self._initialPage is None:
            ...
            self._initialPage, self._initialPageSoup = r, bs4.BeautifulSoup(
                r.content, "lxml", from_encoding=r.encoding
            )
    ...
    def _get_entity(self):
        ...
        if idolsDiv := soup.find("div", id="profile_idols"):
            if (topDiv := idolsDiv.find("div", class_="header_top")) and topDiv.find(
                "span", class_="header_label"
            ).text == "Following":
                kwargs["following"] = snscrape.base.IntWithGranularity(
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            "username",
            type=snscrape.base.nonempty_string("username"),
            help="A VK username",
        )
