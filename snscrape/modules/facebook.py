...
class _FacebookCommonScraper(snscrape.base.Scraper):
    def _clean_url(self, dirtyUrl):
        ...
        if u.path == "/permalink.php":
            ...
            clean = (
                ...
                u.netloc,
                u.path,
                urllib.parse.urlencode((("fbid", q["fbid"][0]),)),
                "",
            )
            ...
            clean = (
                ...
                u.netloc,
                u.path,
                urllib.parse.urlencode((("set", setVal),)),
                "",
            )
        elif (
            u.path.split("/")[2] == "posts"
            or u.path.startswith("/events/")
            or u.path.startswith("/notes/")
            or u.path.split("/")[1:4:2] == ["groups", "permalink"]
        ):
    ...
    def _soup_to_items(self, soup, baseUrl, mode):
        ...
        for entry in soup.find_all(
            "div", class_="_5pcr"
        ):  # also class 'fbUserContent' in 2017 and 'userContentWrapper' in 2019
            ...
            while parent:
                if (
                    ...
                    and "_5pcr" in parent.attrs["class"]
                ):
                    isNested = True
                    break
            ...
            for a in entry.find_all("a"):
                ...
                if (
                    ...
                    or outlink.startswith("https://")
                    and outlink not in outlinks
                ):
...
class _FacebookUserAndCommunityScraper(_FacebookCommonScraper):
    ...
    def get_items(self):
        ...
        while nextPageLink := soup.find("a", ajaxify=nextPageLinkPattern):
            ...
            assert (
                response["domops"][0][0] == "replace"
            ), f'{response["domops"][0]} is not "replace"'
            assert response["domops"][0][1] in (
                "#www_pages_reaction_see_more_unitwww_pages_home",
                "#www_pages_reaction_see_more_unitwww_pages_community_tab",
            )
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            ...
            help="A Facebook username or user ID",
        )
...
class FacebookUserScraper(_FacebookUserAndCommunityScraper):
    ...
    def _get_entity(self):
        ...
        if pageTransparencyContentDiv.text.startswith("Page created - "):
            ...
            kwargs["created"] = datetime.date(
                int(m.group(3)), months.index(m.group(1)) + 1, int(m.group(2))
            )
                ...
                if (
                    ...
                    for a in div.find_all("a"):
                        if a.text == "" or "href" not in a.attrs or a.find("span"):
                            continue
                        dirtyWeb = a["href"]
                        assert dirtyWeb.startswith(
                            "https://l.facebook.com/l.php?u="
                        ), "unexpected web link"
                        kwargs["web"] = urllib.parse.unquote(
...
class FacebookGroupScraper(_FacebookCommonScraper):
    ...
    def get_items(self):
        ...
        if 'content:{pagelet_group_mall:{container_id:"' not in r.text:
            raise snscrape.base.ScraperException(
                "Code container ID marker not found (does the group exist?)"
            )

        soup = bs4.BeautifulSoup(r.text, "lxml")

        # Posts are inside an HTML comment in two code tags with IDs listed in JS...
        for codeContainerIdStart in (
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            "group",
            type=snscrape.base.nonempty_string("group"),
            help="A group name or ID",
        )
