...
class Post(snscrape.base.Item):
    ...
    pictures: typing.Optional[
        typing.List[str]
    ]  # May be shorter than pictureCount if the API didn't return all of them (e.g. post Ipay2evb0)
...
class WeiboUserScraper(snscrape.base.Scraper):
    ...
    def _ensure_user_id(self):
        if self._isUserId:
            return
        r = self._get(
            f"https://m.weibo.cn/n/{self._user}",
            headers=self._headers,
            allowRedirects=False,
        )
        if (
            r.status_code == 302
            and r.headers["Location"].startswith("/u/")
            and len(r.headers["Location"]) == 13
            and r.headers["Location"][3:].strip("0123456789") == ""
        ):
            # Redirect to uid URL
            ...
            raise snscrape.base.ScraperException(
                f"Got unexpected response on resolving username ({r.status_code})"
            )
    ...
    def _mblog_to_item(self, mblog):
        if mblog.get("page_info", {}).get("type") not in (None, "video", "webpage"):
            _logger.warning(
                f'Skipping unknown page info {mblog["page_info"]["type"]!r} on status {mblog["id"]}'
            )
        return Post(
            url=f'https://m.weibo.cn/status/{mblog["bid"]}',
            id=mblog["id"],
            user=self._user_info_to_entity(mblog["user"])
            if mblog["user"] is not None
            else None,
            createdAt=mblog["created_at"],
            text=mblog["raw_text"]
            if "raw_text" in mblog
            else _HTML_STRIP_PATTERN.sub("", mblog["text"]),
            ...
            pictures=[x["large"]["url"] for x in mblog["pics"]]
            if "pics" in mblog
            else None,
            video=urls.get("mp4_720p_mp4")
            or urls.get("mp4_hd_mp4")
            or urls["mp4_ld_mp4"]
            if "page_info" in mblog
            and mblog["page_info"]["type"] == "video"
            and (urls := mblog["page_info"]["urls"])
            else None,
            link=mblog["page_info"]["page_url"]
            if "page_info" in mblog and mblog["page_info"]["type"] == "webpage"
            else None,
            repostedPost=self._mblog_to_item(mblog["retweeted_status"])
    ...
    def get_items(self):
        ...
        while True:
            ...
            r = self._get(
                f"https://m.weibo.cn/api/container/getIndex?type=uid&value={self._user}&containerid=107603{self._user}&count=25{sinceParam}",
                headers=self._headers,
                responseOkCallback=self._check_timeline_response,
            )
            if r.status_code != 200:
    ...
    def _get_entity(self):
        ...
        if self._user is _userDoesNotExist:
            return
        r = self._get(
            f"https://m.weibo.cn/api/container/getIndex?type=uid&value={self._user}",
            headers=self._headers,
        )
        if r.status_code != 200:
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            ...
            dest="isName",
            action="store_true",
            help="Use username instead of user ID",
        )
        subparser.add_argument(
            "user", type=snscrape.base.nonempty_string("user"), help="A user ID"
        )
