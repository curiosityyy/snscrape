__all__ = [
    ...
    "User",
    "CustomEmoji",
    "MastodonProfileScraper",
    "MastodonTootScraperMode",
    "MastodonTootScraper",
]
...
class _MastodonCommonScraper(snscrape.base.Scraper):
    ...
    def _entries_to_items(self, entries, url):
        for entry in entries:
            ...
            tootKwargs["date"] = datetime.datetime.strptime(
                info.find("data", class_="dt-published")["value"],
                "%Y-%m-%dT%H:%M:%S+00:00",
            ...
            if (
                userKwargs["account"].count("@") == 1
            ):  # Ancient versions don't include the instance for posts from accounts on the instance itself
                userKwargs["account"] = self._url_to_account(userLink["href"])
            userKwargs["_url"] = urllib.parse.urljoin(url, userLink["href"])
            (
                userKwargs["displayName"],
                userKwargs["displayNameWithCustomEmojis"],
            ) = self._display_name(userNameSpan.find("strong"), url)
            userKwargs["avatarUrl"] = urllib.parse.urljoin(
                url, userLink.find("img", class_="u-photo")["src"]
            )
            ...
            if attachmentsDiv := entry.find("div", class_="attachment-list"):
            ...
            elif mediaGalleryDiv := entry.find(
                "div", attrs={"data-component": "MediaGallery"}
            ):  # Before 2.7.0 (https://github.com/mastodon/mastodon/issues/6714)
                ...
                tootKwargs["attachments"] = attachments
            elif attachmentsDiv := entry.find(
                "div", class_="status__attachments"
            ):  # Before 2.3.0 (commit 2bbf987a)
                attachments = []
                for a in attachmentsDiv.find_all("a"):
                    attachments.append(
                        Attachment(
            ...
            for a in content.find_all("a"):
                ...
                if "mention" in cls and "u-url" in cls:
                    ...
                    mentionedUsers.append(
                        User(account=self._url_to_account(mentionUrl), _url=mentionUrl)
                    )
            ...
            if pollDiv := entry.find("div", attrs={"data-component": "Poll"}):
                ...
                pollKwargs["expirationDate"] = datetime.datetime.strptime(
                    o["poll"]["expires_at"], "%Y-%m-%dT%H:%M:%S.%fZ"
                ).replace(tzinfo=datetime.timezone.utc)
                pollKwargs["multiple"] = o["poll"]["multiple"]
                pollKwargs["options"] = [
                    PollOption(title=op["title"], votesCount=op["votes_count"])
            ...
            if prepend and prepend.find("i", class_="fa-retweet"):  # Is a boost
                ...
                (
                    userKwargs["displayName"],
                    userKwargs["displayNameWithCustomEmojis"],
                ) = self._display_name(userLink.find("strong"), url)
        ...
        for child in strong.children:
            if isinstance(child, bs4.element.NavigableString):
                ...
                outFull.append(
                    CustomEmoji(
                        shortName=child["alt"],
                        url=urllib.parse.urljoin(url, child["data-original"]),
                        staticUrl=urllib.parse.urljoin(url, child["data-static"]),
                    )
                )
                ...
                outFull.append(
                    CustomEmoji(
                        shortName=child["alt"],
                        url=urllib.parse.urljoin(url, child["src"]),
                    )
                )
            else:
...
class MastodonProfileScraper(_MastodonCommonScraper):
    ...
    def get_items(self):
        ...
        while True:
            ...
            nextA = soup.find(
                "a",
                class_="load-more",
                href=lambda x: "?max_id=" in x or "&max_id=" in x,
            )
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            "account",
            type=snscrape.base.nonempty_string("account"),
            help="A Mastodon account. This can be either a URL to the profile page or a string of the form @account@instance.example.org",
        )
...
class MastodonTootScraper(_MastodonCommonScraper):
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            "--thread",
            action="store_true",
            help="Collect thread around the toot referenced by the URL",
        )
        subparser.add_argument(
            "url", type=snscrape.base.nonempty_string("url"), help="A URL for a toot"
        )
