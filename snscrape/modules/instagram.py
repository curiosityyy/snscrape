...
class _InstagramCommonScraper(snscrape.base.Scraper):
        ...
        for node in response[self._responseContainer][self._edgeXToMedia]["edges"]:
            ...
            yield InstagramPost(
                ...
                date=datetime.datetime.fromtimestamp(
                    node["node"]["taken_at_timestamp"], datetime.timezone.utc
                ),
                content=node["node"]["edge_media_to_caption"]["edges"][0]["node"][
                    "text"
                ]
    ...
    def _initial_page(self):
        if self._initialPage is None:
            ...
            r = self._get(
                ...
                responseOkCallback=self._check_initial_page_callback,
            )
    ...
    def get_items(self):
        ...
        while True:
            ...
            headers["X-Instagram-GIS"] = hashlib.md5(
                f"{rhxGis}:{variables}".encode("utf-8")
            ).hexdigest()
            r = self._get(
...
class InstagramUserScraper(_InstagramCommonScraper):
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            ...
            help="An Instagram username (no leading @)",
        )
...
class InstagramHashtagScraper(_InstagramCommonScraper):
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            ...
            help="An Instagram hashtag (no leading #)",
        )
